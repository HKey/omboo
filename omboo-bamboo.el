...
(defun omboo-bamboo--make-item (class &rest keys)
  (let ((not-found (cl-gensym)))
    (when (eq not-found (cl-getf keys :title not-found))
      (error "title parameter is required"))
    (when (eq not-found (cl-getf keys :content not-found))
      (error "content parameter is required"))
    (when (eq not-found (cl-getf keys :url not-found))
      (error "url parameter is required"))
    (when (eq not-found (cl-getf keys :date not-found))
      (error "date parameter is required"))
    (when (eq not-found (cl-getf keys :readed not-found))
      (error "readed parameter is required"))
    ;; (when (eq not-found (cl-getf keys :author not-found))
    ;;   (error "author parameter is required"))
    (when (eq not-found (cl-getf keys :item-id not-found))
...
(defun omboo-bamboo--make-feed (class &rest keys)
  (let ((not-found (cl-gensym)))
    ...
    (when (eq not-found (cl-getf keys :url not-found))
      (error "url parameter is required"))
    (when (eq not-found (cl-getf keys :webpage not-found))
      (error "webpage parameter is required")))
